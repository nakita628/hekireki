...
const harness = resolve(import.meta.dirname, '..', 'harness', 'pydantic')
...
describe('pydantic', () => {
  ...
  // Pydantic v2 ships inline type information (py.typed), so mypy --strict
  // checks the generated BaseModel declarations against the real pydantic API.
  it.skipIf(!hasPython)('type-checks against the real pydantic API', () => {
  ...
  // Validation behavior a type check cannot see: lax coercion, extra="forbid"
  // rejection, extra="allow" retention, alias round-trips, None defaults.
  it.skipIf(!hasPython)('validates against a live pydantic runtime', () => {
