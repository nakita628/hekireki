...
- **Types.** Int, BigInt beyond 2^53, Float, Decimal, Boolean, String with a newline, DateTime,
  ...
  DateTime is written as Prisma Client writes one: in UTC with milliseconds, and on SQLite as ISO
  8601 text, so the rows of both clients compare and sort as text in time order. One given with no
  zone is in the app's timezone, as Eloquent reads it; one the table holds with no zone is UTC.
