...
This builds the generators, wipes `generated/`, runs `prisma generate`, and verifies that all 18 outputs exist — printing a ✓/✗ table and exiting non-zero if any generator produced nothing.
...
- Validator annotations (`@z.` / `@v.` / `@a.` / `@e.` / `@t.` / `@j.`) on `User` and `Post`; the remaining models fall back to the built-in type mapping
