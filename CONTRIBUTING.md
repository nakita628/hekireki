...
`hekireki seed` has its own database-backed check, `test/db/seed.test.ts`, run by the `Seed DB` workflow against PostgreSQL and MySQL service containers. Locally, `docker compose -f examples/compose.yaml up -d` starts the two, and the test reads their URLs from `HEKIREKI_SEED_PG` and `HEKIREKI_SEED_MYSQL` (`postgresql://postgres:postgres@localhost:5432/seed`, `mysql://root:root@localhost:3306/seed`); without them it skips. The projects it runs are `examples/postgresql` and `examples/mysql` (see `examples/README.md`): `schema.prisma` with the `prisma-client` and `hekireki-seed` generators, the schema module the latter writes (the client is generated by the test and not committed), and a typed `hekireki.config.ts` whose `url` names the compose database. The test regenerates both, creates the tables with `prisma db push`, seeds through the built CLI with that config (the banner must say the rows went through the generated Prisma Client, with the `@prisma/adapter-pg` / `@prisma/adapter-mariadb` devDependencies), and checks every foreign key, the one-to-one and unique constraints, the mapped enum values, the PostgreSQL sequences and the `--sql` script variant.
...
├── database/    # where a database URL comes from (--url, hekireki.config.ts, prisma.config.ts, .env), shared by studio and seed
