...
/** A DMMF with only what `makeDocs` reads: one model, its mapping, and the Query/Mutation types. */
function dmmfWith(mapping: Record<string, string>) {
  return {
    datamodel: {
      models: [
        {
          ...
          uniqueFields: [],
          uniqueIndexes: [],
          primaryKey: { name: null, fields: ['tenantId', 'email'] },
          fields: [
            {
              name: 'email',
              kind: 'scalar',
    ...
    mappings: { modelOperations: [{ model: 'User', ...mapping }], otherOperations: {} },
    schema: {
      outputObjectTypes: {
        prisma: [
          {
            ...
            fields: [
              {
                name: 'theLegacyFindName',
                args: [{ name: 'where', isRequired: true, inputTypes: [] }],
                isNullable: true,
...
// Prisma 2 named the single-record operations `findOne` / `createOne` / …; a schema generated by
// that version still has to render, so the current name is only the last resort of each chain.
describe('makeDocs on Prisma 2 operation names', () => {
  it.each(['findOne', 'findSingle'])('reads findUnique out of %s', (key) => {
    const docs = makeDocs({ dmmf: dmmfWith({ [key]: 'theLegacyFindName' }) as never })
    const operation = docs.models[0]?.operations.find((op) => op.name === 'findUnique')
    expect(operation?.output).toStrictEqual({ type: 'User', required: false, list: false })
    expect(operation?.inputs).toStrictEqual([{ name: 'where', types: [], required: true }])
  })

  it('prefers the current name when the mapping carries one', () => {
    const docs = makeDocs({ dmmf: dmmfWith({ findUnique: 'theLegacyFindName' }) as never })
    const operation = docs.models[0]?.operations.find((op) => op.name === 'findUnique')
    expect(operation?.output.type).toBe('User')
  })
  ...
  it.each([
    ...
    ['delete', 'deleteOne'],
    ['delete', 'deleteSingle'],
  ])('leaves %s without a schema field when only %s names it on the Mutation side', (name, key) => {
    const docs = makeDocs({ dmmf: dmmfWith({ [key]: 'theLegacyFindName' }) as never })
    const operation = docs.models[0]?.operations.find((op) => op.name === name)
    // The fixture only declares a Query type, so a Mutation operation finds no field at all.
    expect(operation?.output).toStrictEqual({ type: null, required: true, list: false })
    expect(operation?.inputs).toBeNull()
  })

  it('renders a composite primary key as an @@id directive', () => {
    const docs = makeDocs({ dmmf: dmmfWith({}) as never })
    expect(docs.models[0]?.directives).toStrictEqual([
      { name: '@@id', values: ['tenantId', 'email'] },
    ])
  })
})
