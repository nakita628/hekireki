...
import { makeValibotRelations, valibot as valibotSchema } from '../helper/valibot.js'
import { getBool } from '../utils/index.js'

export async function valibot(options: GeneratorOptions): Promise<void> {
  ...
  const relations = enableRelation
    ? makeRelationsOnly(options.dmmf, getBool(options.generator.config?.type), makeValibotRelations)
    : ''
