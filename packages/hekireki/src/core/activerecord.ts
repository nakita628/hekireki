...
import {
  ...
  prismaDateTimeFiles,
} from '../generator/activerecord.js'
...
export function activerecord(options: GeneratorOptions) {
  return Effect.gen(function* () {
    ...
    const files = [
      ...prismaDateTimeFiles(options.dmmf.datamodel.models, provider),
      ...activeRecordModelFiles(options.dmmf.datamodel.models, enums, provider),
    ]
    ...
    const withBase =
      path.basename(outDir) === 'models' ? [applicationRecordFile(), ...files] : files
